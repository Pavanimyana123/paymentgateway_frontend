...
const Transactions = () => {
  ...
  const filterTransactions = () => {
    ...
    if (gatewayFilter !== "all") {
      filtered = filtered.filter(
        (transaction) => transaction.gateway === gatewayFilter,
      );
    }
    ...
    if (environmentFilter !== "all") {
      filtered = filtered.filter(
        (transaction) => transaction.environment === environmentFilter,
      );
    }
  ...
  const handleSplitPayout = async (transaction) => {
    try {
      const res = await axios.post(`${BaseURL}/api/create-split-payout`, {
        reference_no: transaction.payment_id,
        amount: transaction.amount,
        sub_account_id: "Lavanyamurugan", // or from DB
      });

      console.log("Response", res.data);

      alert("✅ Split payout successful");
      // navigate("/splitpayouts");
    } catch (err) {
      console.error(err);
      alert(err.response?.data?.error || "Split payout failed");
    }
  };
        ...
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            ...
            <p className="text-muted mb-0">
              Total: {filteredTransactions.length} transaction
                      ...
                      <tr key={transaction.id || transaction.order_id}>
                        ...
                        <td>
                          <Badge className="text-uppercase">
                            {transaction.split_payment_status}
                          </Badge>
                        </td>
                        <td>
                          <Badge
                            bg={getEnvironmentBadge(transaction.environment)}
                            className="text-uppercase"
                          >
                            {transaction.environment}
                          </Badge>
                        ...
                        <td className="text-end pe-4">
                          <div className="d-flex justify-content-end gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              disabled={transaction.split_done}
                              onClick={() => handleSplitPayout(transaction)}
                            >
                              Split Payout
                            </Button>
              ...
              <Row className="g-3">
                ...
                <Col md={6}>
                  <div className="mb-3">
                    ...
                    <Badge
                      bg={getEnvironmentBadge(selectedTransaction.environment)}
                      className="text-uppercase"
                    >
                ...
                <Col md={6}>
                  <div className="mb-3">
                    ...
                    <Badge
                      bg={getStatusBadge(selectedTransaction.status)}
                      className="text-uppercase"
                    >
